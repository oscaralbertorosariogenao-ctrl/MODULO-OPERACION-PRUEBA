...
export function agencyDetailView(state){
  ...
  return el('div',{class:'page agency-detail-page'},
    ...
    can('operations.create',state) ? el('button',{class:'btn btn-primary btn-block',type:'button','data-action':'create-operation-from-agency','data-agency-id':agency.id || agency.numero},'＋ Reportar problema en esta agencia') : null,
    can('operations.view',state) ? section(`Operaciones activas (${relatedOps.length})`,relatedOps.length ? el('div',{class:'list'},relatedOps.map(operationCard)) : el('p',{class:'muted',text:'No hay operaciones activas vinculadas.'})) : null,
    can('equipment.view',state) ? section(`Ficha técnica móvil (${equipment.length})`,equipment.length ? el('div',{class:'list'},equipment.map(item => el('div',{class:'list-item'},el('span',{'aria-hidden':'true',text:'▣'}),el('div',{class:'list-main'},el('strong',{text:item.productos?.nombre || item.producto_nombre || item.serial || 'Equipo'}),el('p',{text:`Serial: ${item.serial || 'No aplica'} · Estado: ${item.estado || 'No registrado'}`}))))) : el('p',{class:'muted',text:'No hay equipos activos visibles para esta agencia.'})) : null
  );
}
