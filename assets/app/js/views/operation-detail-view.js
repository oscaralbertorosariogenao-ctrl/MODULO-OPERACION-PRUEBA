...
export function operationDetailView(state, canAction){
  ...
  return el('div',{class:'page operation-detail-page'},
    ...
    section('Seguimiento',el('div',{class:'grid grid-2'},
      ...
      actionButton('⚠','Reportar problema encontrado','report-related-problem',canAction('operations.report') && Boolean(op.agencyNumber) && isActiveOperationStatus(op.status)),actionButton('☎','WhatsApp','open-whatsapp-actions',Boolean(op.managerPhone)),actionButton('⌂','Abrir agencia','open-operation-agency',Boolean(op.agencyNumber))
    )),
    ...
    canAction('operations.closeWhatsapp') && !isTerminalOperationStatus(op.status) ? el('button',{class:'btn btn-outline btn-block',type:'button','data-action':'open-whatsapp-close'},'Cerrar por soporte WhatsApp') : null,
