...
export function profileView(state){
  ...
  const installLabel = state.ui.pwaIos && !state.ui.installPrompt ? 'Cómo instalar en iPhone' : 'Instalar aplicación';
  return el('div',{class:'page profile-page'},
    ...
    section('Aplicación',el('dl',{class:'info-list'},info('Conexión',state.connectivity.online ? 'En línea' : 'Sin conexión'),info('Modo',state.ui.pwaStandalone ? 'Aplicación instalada' : 'Navegador'),info('Última sincronización',formatDate(state.connectivity.lastSync)),info('Versión',APP_VERSION),info('Build',APP_BUILD))),
    ...
    state.ui.pwaUpdateAvailable ? el('button',{class:'btn btn-primary btn-block',type:'button','data-action':'activate-pwa-update'},'Actualizar aplicación') : null,
    !state.ui.pwaStandalone && (state.ui.installPrompt || state.ui.pwaIos) ? el('button',{class:'btn btn-outline btn-block',type:'button','data-action':'install-pwa'},installLabel) : null,
