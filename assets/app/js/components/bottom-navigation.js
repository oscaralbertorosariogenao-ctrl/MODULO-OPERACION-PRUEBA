...
const ITEMS = [
  ...
  {path:'/group-inventory',label:'Inventario',icon:'▣',action:'go-group-inventory',permission:'groupInventory.view',groupOnly:true},
  {path:'/agencies',label:'Agencias',icon:'⌖',action:'go-agencies',permission:'agencies.view',hideForGroupManager:true},
...
export function bottomNavigation(state){
  ...
  const visibleItems = ITEMS.filter(item => can(item.permission,state) && (!item.groupOnly || groupManager) && !(item.hideForGroupManager && groupManager));
