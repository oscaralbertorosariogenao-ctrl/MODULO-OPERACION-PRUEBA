...
export async function loadHomeData(){
  const [statsRows,recent,notificationsResult,technicians] = await Promise.all([
    getOperationsForStats(),getRecentOperations(8),listNotifications(40).catch(error => ({__error:error})),listTechnicians().catch(() => [])
  ]);
  const realNotifications = Array.isArray(notificationsResult) ? notificationsResult : [];
  const derived = deriveOperationalAlerts(statsRows);
  const technicianMap = new Map();
  for(const technician of technicians){ const name = technician.nombre_completo || technician.nombre || technician.usuario_login; technicianMap.set(name,{...technician,activeOperations:0,lastActivity:null}); }
  for(const row of statsRows.map(normalizeOperation)){
    if(!['Asignada','En proceso'].includes(row.status)) continue;
    const item = technicianMap.get(row.technician); if(item){ item.activeOperations += 1; if(!item.lastActivity || new Date(row.createdAt) > new Date(item.lastActivity)) item.lastActivity = row.createdAt; }
  }
  ...
  updateSlice('notifications',{items:[...derived,...realNotifications],loading:false,real:Array.isArray(notificationsResult)},'home-notifications');
...
export async function loadTechniciansData(){
  ...
  const [technicians,rows] = await Promise.all([listTechnicians(),getOperationsForStats()]);
  const normalized = rows.map(normalizeOperation);
  const items = technicians.map(tech => { const name=tech.nombre_completo || tech.nombre || tech.usuario_login; const active=normalized.filter(op => ['Asignada','En proceso'].includes(op.status) && op.technician === name); return {...tech,activeOperations:active.length,lastActivity:active[0]?.createdAt || null}; });
...
export async function loadNotificationsData(){
  ...
  const rows = await getOperationsForStats();
  let real = []; let realInfrastructure = true;
  try{ real = await listNotifications(80); }catch{ realInfrastructure = false; }
  updateSlice('notifications',{items:[...deriveOperationalAlerts(rows),...real],loading:false,real:realInfrastructure},'notifications-loaded'); markSync();
}
