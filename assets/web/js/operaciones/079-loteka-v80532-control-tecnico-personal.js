(function (global) {
  ...
  function applyFilters() {
    ...
    const priority = $('#goct-priority')?.value || '';
    ...
    state.filtered = state.items.filter((item) => {
      ...
      if (priority && item.prioridad !== priority) return false;
      if (query) {
  ...
  function editItem(id) {
    ...
    $('#goct-f-equipment').value = item.equipo || ''; $('#goct-f-state').value = item.estado; $('#goct-f-priority').value = item.prioridad || 'MEDIA';
  ...
  async function saveItem() {
    ...
    try {
      ...
      const payload = {
        ...
        estado: selectedStatus, prioridad: $('#goct-f-priority').value, fecha_reportada: $('#goct-f-date').value || today(),
  ...
  function parseImport() {
    ...
    state.importRows = text($('#goct-import-text').value).split(/\n+/).map(text).filter(Boolean).map((line, index) => {
      ...
      return { line: index + 1, valid: Boolean(agency), agency, agencia_numero: agencyNumber, grupo_codigo: group ? groupLabel(group) : (groupMatch ? `G-${groupMatch[1]}` : '-'), categoria: category, equipo: equipment, asunto: subject, fecha_reportada: reportedDate || today(), estado: /cambiar/i.test(line) ? 'REQUIERE_CAMBIO' : /verificar/i.test(line) ? 'POR_VERIFICAR' : 'PENDIENTE', prioridad: 'MEDIA' };
    });
    renderImportPreview();
  }
