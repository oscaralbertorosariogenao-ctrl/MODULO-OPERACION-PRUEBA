(function(global){
  ...
  if(global.GOOperationalRoutes && global.GOOperationalRoutes.version === '805.26.0') return;

  var VERSION = '805.26.0';
  ...
  function bindEvents(){
    ...
    qs('#gorMapCurrentBtn')?.addEventListener('click',function(){ if(state.comparison) openMap(orderedAgencies(),'Vista previa de la ruta'); });
  ...
  function closeModal(id){var m=document.getElementById(id);if(m){m.classList.remove('open');m.setAttribute('aria-hidden','true');}if(id==='gorMapModal'&&state.map){try{state.map.remove();}catch(_e){}state.map=null;}}
  function openMap(routeAgencies,title){
    var list=(routeAgencies||[]).filter(Boolean);openModal('gorMapModal');qs('#gorMapTitle').textContent=title||'Mapa de ruta';var fallback=qs('#gorMapFallback');fallback.innerHTML='';var coords=list.map(function(a){var c=agencyCoords(a);return c?{a:a,lat:c.lat,lng:c.lng}:null;}).filter(Boolean);
    if(!global.maplibregl||!coords.length){qs('#gorMapCanvas').innerHTML='<div class="gor-empty" style="margin:20px">No hay coordenadas suficientes para dibujar el mapa.</div>';fallback.innerHTML='<div class="gor-chips">'+list.map(function(a){return '<a class="gor-chip" href="'+esc(agencyMapUrl(a))+'" target="_blank">AG '+esc(agencyNumber(a))+'</a>';}).join('')+'</div>';return;}
    try{if(state.map)state.map.remove();state.map=new global.maplibregl.Map({container:'gorMapCanvas',style:{version:8,sources:{osm:{type:'raster',tiles:['https://tile.openstreetmap.org/{z}/{x}/{y}.png'],tileSize:256,attribution:'© OpenStreetMap'}},layers:[{id:'osm',type:'raster',source:'osm'}]},center:[coords[0].lng,coords[0].lat],zoom:11});state.map.addControl(new global.maplibregl.NavigationControl(),'top-right');var bounds=new global.maplibregl.LngLatBounds();coords.forEach(function(x,index){var el=document.createElement('div');el.style.cssText='width:30px;height:30px;border-radius:50%;background:#0689c4;color:white;border:3px solid white;box-shadow:0 3px 10px rgba(0,0,0,.32);display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:1000';el.textContent=String(index+1);var popup=new global.maplibregl.Popup({offset:20}).setHTML('<strong>AG '+esc(agencyNumber(x.a))+'</strong><br>'+esc(agencyName(x.a))+'<br>'+esc(agencyGroupCode(x.a))+'<br><a href="'+esc(agencyMapUrl(x.a))+'" target="_blank">Abrir Google Maps</a>');new global.maplibregl.Marker({element:el}).setLngLat([x.lng,x.lat]).setPopup(popup).addTo(state.map);bounds.extend([x.lng,x.lat]);});state.map.on('load',function(){state.map.resize();if(coords.length>1)state.map.fitBounds(bounds,{padding:55,maxZoom:15});});fallback.innerHTML='<div style="color:#6e8495;font-size:12px;margin-top:8px">'+coords.length+' de '+list.length+' agencias tienen coordenadas disponibles.</div>';}
