(function(global){
  ...
  function renderMapEditor(){
    ...
    state.mapMarkers.forEach(function(m){var idx=ed.order.findIndex(function(a){return agencyKey(a.numero||a.codigo)===m.key;}),selected=ed.selectedKey===m.key; if(m.badgeEl)m.badgeEl.textContent=String(idx+1); if(m.containerEl)m.containerEl.classList.toggle('selected',selected); if(m.markerEl)m.markerEl.classList.toggle('selected',selected); if(m.labelStrong)m.labelStrong.textContent='AG '+agencyNumber(ed.order[idx]||m.agency); if(m.labelSmall)m.labelSmall.textContent=agencyGroupCode(ed.order[idx]||m.agency)||'Sin grupo'; var start=idx===0,end=idx===ed.order.length-1; if(m.containerEl){m.containerEl.classList.toggle('start',start);m.containerEl.classList.toggle('end',end);} if(m.markerEl){m.markerEl.classList.toggle('start',start);m.markerEl.classList.toggle('end',end);} });
    drawMapRouteLine();
  }
  ...
  function routeRenderWarn(error,context){
    try{if(global.console&&console.warn)console.warn('[Rutas reales] '+context,error);}catch(_e){}
  }
  function removeLayerSafe(id){
    if(!state.map)return;
    try{if(state.map.getLayer(id))state.map.removeLayer(id);}catch(error){routeRenderWarn(error,'removeLayer '+id);}
  }
  function clearFixedSegmentLayers(){
    if(!state.map)return;
    try{
      var style=state.map.getStyle&&state.map.getStyle(),layers=style&&Array.isArray(style.layers)?style.layers.slice():[];
      layers.forEach(function(layer){if(layer&&String(layer.id||'').indexOf('gor-route-segment-fixed-')===0)removeLayerSafe(layer.id);});
    }catch(error){routeRenderWarn(error,'limpiar capas de tramos');}
    ['gor-route-segments-arrows','gor-route-segments-layer','gor-route-segments-outline'].forEach(removeLayerSafe);
  }
  function setRouteSource(data){
    ...
    try{
      ...
      if(!state.map.getLayer('gor-route-line-outline'))state.map.addLayer({id:'gor-route-line-outline',type:'line',source:'gor-route-line',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#ffffff','line-width':7,'line-opacity':.22,'line-blur':.7}});
      if(!state.map.getLayer('gor-route-line-layer'))state.map.addLayer({id:'gor-route-line-layer',type:'line',source:'gor-route-line',layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#087fba','line-width':3.8,'line-opacity':.28,'line-dasharray':[1.5,1.5]}});
    }catch(error){routeRenderWarn(error,'dibujar guía de ruta');}
  }
  ...
  function drawMapRouteLine(){
    ...
    setRouteSource(routeFeature(straight));
    var hasRoad=!!(state.roadRoute&&Array.isArray(state.roadRoute.segments)&&state.roadRoute.segments.length);
    var segments=roadRouteSegmentsGeoJSON();
    var rendered=setSegmentSource(segments,hasRoad);
    try{
      if(state.map.getLayer('gor-route-line-layer'))state.map.setLayoutProperty('gor-route-line-layer','visibility',hasRoad?'none':'visible');
      if(state.map.getLayer('gor-route-line-outline'))state.map.setLayoutProperty('gor-route-line-outline','visibility',hasRoad?'none':'visible');
    }catch(error){routeRenderWarn(error,'alternar guía / ruta real');}
    if(hasRoad&&state.roadRoute)state.roadRoute.renderedSegments=rendered;
  }
  ...
  function openMap(routeAgencies,title,options){
    ...
    try{
      ...
      coords.forEach(function(x,index){var key=agencyKey(x.a.numero||x.a.codigo),container=document.createElement('div'),badge=document.createElement('div'),label=document.createElement('div'),strong=document.createElement('strong'),small=document.createElement('span');container.className='gor-map-pin'+(index===0?' start':'')+(index===coords.length-1?' end':'');badge.className='gor-map-marker'+(index===0?' start':'')+(index===coords.length-1?' end':'');badge.textContent=String(index+1);label.className='gor-map-label';strong.textContent='AG '+agencyNumber(x.a);small.textContent=agencyGroupCode(x.a)||'Sin grupo';label.appendChild(strong);label.appendChild(small);container.appendChild(badge);container.appendChild(label);container.addEventListener('click',function(){selectMapAgency(key,false);});var popup=new global.maplibregl.Popup({offset:22}).setHTML('<strong>AG '+esc(agencyNumber(x.a))+'</strong><br>'+esc(agencyName(x.a))+'<br>'+esc(agencyGroupCode(x.a))+'<br><small>'+esc(agencyAddress(x.a)||'Sin dirección')+'</small><br><button type="button" onclick="window.GOOperationalRoutes.moveMapAgencyToPosition(\''+esc(key)+'\')" style="margin-top:7px;padding:6px 8px;border-radius:8px;border:1px solid #bcd3e0;background:#fff;color:#075d94;font-weight:800;cursor:pointer">Cambiar posición</button><br><a href="'+esc(agencyMapUrl(x.a))+'" target="_blank">Abrir Google Maps</a>');var marker=new global.maplibregl.Marker({element:container,anchor:'bottom-left'}).setLngLat([x.lng,x.lat]).setPopup(popup).addTo(state.map);state.mapMarkers.push({key:key,agency:x.a,containerEl:container,markerEl:badge,badgeEl:badge,labelStrong:strong,labelSmall:small,marker:marker});bounds.extend([x.lng,x.lat]);});
      state.map.on('load',function(){state.map.resize();if(coords.length>1)state.map.fitBounds(bounds,{padding:{top:70,right:80,bottom:70,left:80},maxZoom:15});renderMapEditor();drawMapRouteLine();});fallback.innerHTML='<div style="color:#6e8495;font-size:12px;margin-top:8px">'+coords.length+' de '+list.length+' agencias tienen coordenadas disponibles. Cada tramo tiene un color distinto y cada agencia muestra su código sin tocarla.</div>';
