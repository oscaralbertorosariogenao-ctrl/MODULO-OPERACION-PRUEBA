(function(global){
  ...
  function clickHandler(e){const save=e.target.closest('#saveOperationBtn');if(save){createReport(e);return;}const action=e.target.closest('[data-v808-action]');if(!action)return;e.preventDefault();const op=findOperation(action.dataset.op);if(!op)return;if(action.dataset.v808Action==='view'){global.showDetail?.(operationReference(op));enhanceDetail(op);}if(action.dataset.v808Action==='assign')openAssign(op).catch(err=>toast('No se pudo asignar',err.message||String(err),'warning'));if(action.dataset.v808Action==='remote')openRemote(op);}
