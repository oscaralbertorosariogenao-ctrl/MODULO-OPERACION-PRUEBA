(function(global){
  ...
  async function loadTechnicians(){
    if(techCache)return techCache;const client=sb();if(!client)throw new Error('Supabase no disponible.');
    const {data,error}=await client.from('perfiles').select('id,nombre_completo,nombre,usuario_login,activo,roles(nombre),puestos(nombre)').eq('activo',true).order('nombre_completo');if(error)throw error;
    const relationName=value=>{const row=Array.isArray(value)?value[0]:value;return txt(row?.nombre);};
    techCache=(data||[]).filter(p=>/tecnic/i.test(`${relationName(p.roles)} ${relationName(p.puestos)}`)).map(p=>({id:p.id,name:p.nombre_completo||p.nombre||p.usuario_login||'Responsable'}));return techCache;
  }
  ...
  function installStyles(){if(document.getElementById('v808Styles'))return;const st=document.createElement('style');st.id='v808Styles';st.textContent=`.v808-status{display:inline-flex;padding:5px 9px;border-radius:999px;font-weight:700;font-size:.78rem}.v808-status.reported{background:#fff3cd;color:#664d03}.v808-status.assigned{background:#cff4fc;color:#055160}.v808-status.progress{background:#cfe2ff;color:#084298}.v808-status.danger{background:#f8d7da;color:#842029}.v808-status.success{background:#d1e7dd;color:#0f5132}.v808-timer,.v808-related{display:block;margin-top:5px;color:#64748b}.v808-backdrop{position:fixed;inset:62px 0 0;background:rgba(15,23,42,.6);z-index:1000025;display:grid;place-items:start center;padding:18px;overflow:auto}.v808-modal{width:min(560px,100%);max-height:calc(100dvh - 98px);background:#fff;border-radius:16px;box-shadow:0 24px 70px rgba(0,0,0,.28);overflow:auto;margin:0 auto}@media(max-width:700px){.v808-backdrop{inset:58px 0 0;padding:10px}.v808-modal{max-height:calc(100dvh - 76px)}}.v808-head{display:flex;justify-content:space-between;align-items:center;padding:18px 20px;border-bottom:1px solid #e5e7eb}.v808-head h3{margin:0}.v808-head button{border:0;background:transparent;font-size:28px;cursor:pointer}.v808-body{padding:20px;display:grid;gap:16px}.v808-actions{display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap}`;document.head.appendChild(st);}
  ...
  function clickHandler(e){const save=e.target.closest('#saveOperationBtn');if(save){createReport(e);return;}const action=e.target.closest('[data-v808-action]');if(!action)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();const op=findOperation(action.dataset.op);if(!op)return;if(action.dataset.v808Action==='view'){global.showDetail?.(operationReference(op));enhanceDetail(op);}if(action.dataset.v808Action==='assign')openAssign(op).catch(err=>toast('No se pudo asignar',err.message||String(err),'warning'));if(action.dataset.v808Action==='remote')openRemote(op);}
