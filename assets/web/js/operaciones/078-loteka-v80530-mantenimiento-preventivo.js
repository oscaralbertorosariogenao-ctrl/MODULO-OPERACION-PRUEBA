...
function uuid(v){return /^[0-9a-f-]{36}$/i.test(tx(v))} function pad(v){var d=tx(v).replace(/\D/g,'');return d?(d.length<4?d.padStart(4,'0'):d):''}
function rt(){return global.GOApp&&global.GOApp.__phase2aRuntime?global.GOApp:null}
function db(){var r=rt();if(r)try{var c=r.supabase.getClient();if(c&&c.from)return c}catch(e){};return global.lotekaSupabase||global.supabaseClient||global.__supabaseClient||null}
function toast(m,t){try{if(global.showToast)return global.showToast(m,t||'info')}catch(e){};(t==='error'?console.error:console.log)('[Mantenimiento]',m)}
function err(e){toast((e&&e.message)||String(e||'No se pudo completar la acción.'),'error')}
function profile(){var r=rt(),p=r?(r.state.get('perfil')||{}):{};return [p.rol_nombre,p.rol,p.puesto_nombre,p.puesto].join(' ').toLowerCase()}
function perms(){var r=rt(),x=r?r.state.get('permissions'):[];return new Set(Array.isArray(x)?x.map(String):[])}
function canView(){return perms().has('ver_mantenimiento_preventivo')||perms().has('gestionar_mantenimiento_preventivo')||/administrador|auxiliar de operaciones|gerente|supervisor/.test(profile())}
function canManage(){return perms().has('gestionar_mantenimiento_preventivo')||/administrador|auxiliar de operaciones|gerente de operaciones/.test(profile())}
function agencies(){return (Array.isArray(global.agencias)?global.agencias:[]).filter(function(a){return a.activo!==false&&!/cerrad|inactiv|desactiv/i.test(tx(a.estado||a.estado_operativo))})}
function groups(){return (Array.isArray(global.grupos)?global.grupos:[]).filter(function(g){return g.activo!==false&&!/prueba|test|cerrad|desactiv/i.test(tx(g.nombre||g.codigo))})}
function aid(a){for(var x of [a&&a.supabaseId,a&&a.id_supabase,a&&a.agencia_id,a&&a.id])if(uuid(x))return tx(x);return''}
function gid(g){for(var x of [g&&g.supabaseId,g&&g.id_supabase,g&&g.grupo_id,g&&g.id])if(uuid(x))return tx(x);return''}
...
function progress(p){var a=p.mantenimiento_plan_agencias||[],done=a.filter(x=>x.estado==='COMPLETADO').length;return {done:done,total:a.length,pct:a.length?Math.round(done/a.length*100):0}}
function renderPlans(){var f=tx(q('#gomp-filter-status')&&q('#gomp-filter-status').value),list=state.planes.filter(x=>!f||x.estado===f),el=q('#gomp-plans');if(!list.length){el.innerHTML='<div class="gomp-empty">No hay planes con este filtro.</div>';return}el.innerHTML='<div style="overflow:auto"><table class="gomp-table"><thead><tr><th>Código</th><th>Plan</th><th>Tipo</th><th>Fecha</th><th>Responsable</th><th>Avance</th><th>Estado</th><th></th></tr></thead><tbody>'+list.map(function(p){var z=progress(p);return '<tr><td><b>'+esc(p.codigo||'-')+'</b></td><td>'+esc(p.nombre)+'</td><td>'+esc(p.mantenimiento_tipos&&p.mantenimiento_tipos.nombre)+'</td><td>'+fmtDate(p.fecha_inicio)+'</td><td>'+esc(p.responsable_nombre||'Sin asignar')+'</td><td><b>'+z.done+'/'+z.total+'</b><div class="gomp-progress"><i style="width:'+z.pct+'%"></i></div></td><td><span class="gomp-badge '+statusClass(p.estado)+'">'+esc(statusLabel(p.estado))+'</span></td><td><button class="gomp-btn" data-detail="'+p.id+'">Abrir</button></td></tr>'}).join('')+'</tbody></table></div>';qa('[data-detail]',el).forEach(b=>b.onclick=()=>showDetail(b.dataset.detail))}
function renderTypes(){var el=q('#gomp-types');el.innerHTML=state.tipos.length?state.tipos.map(t=>'<article class="gomp-type"><h4>'+esc(t.nombre)+'</h4><p>'+esc(t.descripcion||'Sin descripción')+'</p><small>'+(t.frecuencia_dias?('Cada '+t.frecuencia_dias+' días'):'Frecuencia variable')+' · '+(t.evidencia_obligatoria?'Con evidencia':'Evidencia opcional')+'</small></article>').join(''):'<div class="gomp-empty">No hay tipos registrados.</div>'}
function renderCalendar(){var list=state.planes.slice().sort((a,b)=>tx(a.fecha_inicio).localeCompare(tx(b.fecha_inicio))),el=q('#gomp-calendar');el.innerHTML=list.length?'<table class="gomp-table"><thead><tr><th>Fecha</th><th>Plan</th><th>Agencias</th><th>Estado</th></tr></thead><tbody>'+list.map(p=>'<tr><td>'+fmtDate(p.fecha_inicio)+'</td><td><b>'+esc(p.nombre)+'</b><br><small>'+esc(p.codigo)+'</small></td><td>'+((p.mantenimiento_plan_agencias||[]).length)+'</td><td><span class="gomp-badge '+statusClass(p.estado)+'">'+esc(statusLabel(p.estado))+'</span></td></tr>').join('')+'</tbody></table>':'<div class="gomp-empty">No hay fechas programadas.</div>'}
function fillFormOptions(){var t=q('#gomp-type');if(t)t.innerHTML=state.tipos.map(x=>'<option value="'+x.id+'">'+esc(x.nombre)+'</option>').join('');var g=q('#gomp-group-filter');if(g){g.innerHTML='<option value="ALL">Todos los grupos</option>'+groups().sort((a,b)=>glabel(a).localeCompare(glabel(b),'es',{numeric:true})).map(x=>'<option value="'+gid(x)+'">'+esc(glabel(x))+'</option>').join('')}}
function openPlan(){if(!canManage())return toast('No tienes permiso para crear planes.','error');state.selected.clear();q('#gomp-name').value='';q('#gomp-start').value=new Date().toISOString().slice(0,10);q('#gomp-limit').value='';q('#gomp-responsible').value='';q('#gomp-notes').value='';q('#gomp-agency-search').value='';q('#gomp-group-filter').value='ALL';renderPicker();q('#gomp-plan-modal').classList.add('open')}
function visibleAgencies(){var search=tx(q('#gomp-agency-search').value).toLowerCase(),gf=q('#gomp-group-filter').value;return agencies().filter(function(a){if(gf!=='ALL'&&agid(a)!==gf)return false;var g=groupForAgency(a);return !search||[anum(a),aname(a),tx(a.direccion),glabel(g)].join(' ').toLowerCase().includes(search)}).sort((a,b)=>Number(anum(a))-Number(anum(b)))}
function renderPicker(){var list=visibleAgencies(),el=q('#gomp-agency-picker');el.innerHTML=list.map(function(a){var g=groupForAgency(a),id=aid(a);return '<label class="gomp-ag"><input type="checkbox" data-ag="'+id+'" '+(state.selected.has(id)?'checked':'')+'><b>AG '+anum(a)+'</b><span>'+esc(aname(a))+'</span><small>'+esc(glabel(g))+'</small></label>'}).join('')||'<div class="gomp-empty">No hay agencias.</div>';qa('[data-ag]',el).forEach(x=>x.onchange=function(){this.checked?state.selected.add(this.dataset.ag):state.selected.delete(this.dataset.ag);updateSelected()});updateSelected()}
